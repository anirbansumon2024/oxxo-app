...
public class DetailsActivity extends AppCompatActivity implements CastPlayer.SessionAvailabilityListener, ProgramAdapter.OnProgramClickListener, EpisodeAdapter.OnTVSeriesEpisodeItemClickListener,
    ...
    protected void onStart() {
        ...
        if (mAudioManager != null) {
            ...
            volumnSeekbar.setProgress(currentVolume);
        }
            ...
            if (!listServer.isEmpty()) {
                if (isPaid) {
                    if (PreferenceUtils.isActivePlan(DetailsActivity.this)) {
                        ...
                        // user doesn't have any active plan
                        // show subscription dialog to user
    ...
    public void createDownloadDir() {
        File file = new File(Constants.getDownloadDir(DetailsActivity.this), getResources().getString(R.string.app_name));
        if (!file.exists()) {
            file.mkdirs();
        }
    }
    ...
    public void initVideoPlayer(String url, Context context, String type) {
        ...
        switch (type) {
            ...
            case YOUTUBE_LIVE:
                ...
                ContinueWatchingModel model = new ContinueWatchingModel(id, title,
                        castImageUrl, 0, resumePosition, url,
                        categoryType, type);

                Intent youtubeIntent = new Intent(DetailsActivity.this, YoutubePlayerActivity2.class);
                youtubeIntent.putExtra("continueWatchingData", model);
    ...
    private void initYoutubePlayer(String url) {
        ...
        transaction.commit();
        fragment.initialize("AppConfig.YOUTUBE_API_KEY"
                ...
                if (response.code() == 200) {
                    if (response.body() != null) {
                        ...
                        if (detailsModel.getStreamFrom().equalsIgnoreCase("youtube") || detailsModel.getStreamFrom().equalsIgnoreCase("youtube_live")) {
                            ContinueWatchingModel data = new ContinueWatchingModel(id, title,
                                    castImageUrl, 0, resumePosition, detailsModel.getStreamUrl(),
                                    categoryType, "youtube");

                            Intent youtubeIntent = new Intent(DetailsActivity.this, YoutubePlayerActivity2.class);
                            youtubeIntent.putExtra("continueWatchingData", data);
                            startActivity(youtubeIntent);
                        } else {
